...
volumeloc = f"/Volumes/{catalog_name}/bronze/youtube_earthquake_volume"
primary_key="id"
...
@dp.table(name='earth_quakes_silver')
def sample_users_restapiingesttosilver():
    df=spark.readStream.format("cloudFiles").option("cloudFiles.format", "json").load(volumeloc).withColumn("_load_timestamp", current_timestamp())
